<template lang='pug'>
  .login
    .auth-layout_inner
      q-card.auth-layout_card.card
        header.auth-layout_card-header.card-header
          img.logo(src="/images/logo.svg" width="60" height="60" alt="Logo")
          h2 Login
        span.divider
        ...
        <!--  el-form-item-->
        <!--    el-button.auth-layout_submit.full-width(size="large" type="primary" @click="submitForm('loginForm')" :loading="isLoading")-->
        <!--      | Enter-->
        <!--  el-form-item.item-forgot-password-->
        <!--    router-link(:to="{ name: 'forgot-password'}") Forgot password?-->
      footer.auth-layout_footer
        | No account yet?
